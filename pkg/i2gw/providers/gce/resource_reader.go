...
func (r *reader) readResourcesFromCluster(ctx context.Context) (*storage, error) {
	...
	backendConfigs, betaBackendConfigs, err := r.readBackendConfigsFromCluster(ctx)
	if err != nil {
		return nil, err
	}
	storage.BackendConfigs = backendConfigs
	storage.BetaBackendConfigs = betaBackendConfigs
	return storage, nil
}
...
func (r *reader) readBackendConfigsFromCluster(ctx context.Context) (map[types.NamespacedName]*backendconfigv1.BackendConfig, map[types.NamespacedName]*backendconfigv1beta1.BackendConfig, error) {
	...
	var betaBackendConfigList backendconfigv1beta1.BackendConfigList
	betaBackendConfigList.SetGroupVersionKind(betaBackendConfigGVK)
	err = r.conf.Client.List(ctx, &betaBackendConfigList)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get backendConfigs from the cluster: %w", err)
	}
	betaBackendConfigs := make(map[types.NamespacedName]*backendconfigv1beta1.BackendConfig)
	for i, betaBackendConfig := range betaBackendConfigList.Items {
		betaBackendConfigs[types.NamespacedName{Namespace: betaBackendConfig.Namespace, Name: betaBackendConfig.Name}] = &betaBackendConfigList.Items[i]
	}

	return backendConfigs, betaBackendConfigs, nil
}
