...
type ResourcesToIRConverter interface {
	Convert(Storage) (i2gw.IR, field.ErrorList)
}
...
func (c *resourcesToIRConverter) Convert(storage Storage) (i2gw.IR, field.ErrorList) {
	...
	for _, spec := range storage.GetResources() {
		...
		for _, httpRoute := range httpRoutes {
			ir.HTTPRoutes[types.NamespacedName{Name: httpRoute.GetName(), Namespace: httpRoute.GetNamespace()}] = i2gw.HTTPRouteContext{HTTPRoute: httpRoute}
		}
		...
		for _, gateway := range gateways {
			ir.Gateways[types.NamespacedName{Name: gateway.GetName(), Namespace: gateway.GetNamespace()}] = i2gw.GatewayContext{Gateway: gateway}
